...
func ClusterUp(c *cli.Context) {
	...
	err = createCluster(c, ecsClient, cfnClient, commandConfig)
	if err != nil {
		logrus.Fatal("Error executing 'up': ", err)
	}
...
func createCluster(context *cli.Context, ecsClient ecsclient.ECSClient, cfnClient cloudformation.CloudformationClient, commandConfig *config.CommandConfig) error {
	var err error
	...
	cfnClient.Initialize(commandConfig)
	...
	if err == cloudformation.ParameterNotFoundError {
		amiId, err := amiIds.Get(aws.StringValue(commandConfig.Session.Config.Region))
		if err != nil {
			return err
		}
