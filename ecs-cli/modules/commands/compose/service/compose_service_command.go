...
func loadBalancerFlags() []cli.Flag {
	...
	targetGroupsUsageString := fmt.Sprintf("[Optional] Specifies multiple target groups to register with a service. Can be used with --%s flag or --%s at the same time. To specify multiple target groups, add multiple seperate --%s flags Example: --target-groups \"targetGroupArn=arn,containerName=nginx,containerPort=80\" --target-groups \"targetGroupArn=arn,containerName=database,containerPort=3306\"", flags.LoadBalancerNameFlag, flags.TargetGroupArnFlag, flags.TargetGroupsFlag)
